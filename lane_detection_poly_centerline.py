...
# -----------------------------------------------------------
# Helper function: create region of interest mask
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# Polynomial fit and lane drawing + centerline prediction
# -----------------------------------------------------------
def draw_lane_poly(frame, binary_img):
    ...
    if len(left_fitx) and len(right_fitx):
        ...
        # Draw center trajectory line
        center_x = (left_fitx + right_fitx) / 2
        center_points = np.array([np.transpose(np.vstack([center_x, plot_y]))], np.int32)
        cv2.polylines(lane_overlay, center_points, isClosed=False, color=(255, 0, 0), thickness=4)

    combo = cv2.addWeighted(frame, 1, lane_overlay, 0.4, 0)
    return combo
...
out = cv2.VideoWriter('lane_output_centerline.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (width, height))
cap.set(cv2.CAP_PROP_POS_FRAMES, 0)

print("🚗 Lane Detection with Centerline started... Press 'Q' to stop.")
...
while True:
    ...
    cv2.imshow("Lane Detection + Centerline", lane_result)
...
print("✅ Process completed. Output saved as 'lane_output_centerline.mp4'")
